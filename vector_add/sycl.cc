...
template <typename A_AccessorType, typename B_AccessorType, typename C_AccessorType>
struct vector_add {
public:
  vector_add(A_AccessorType A_, B_AccessorType B_, C_AccessorType C_) :
    A{std::move(A_)},
    B{std::move(B_)},
    C{std::move(C_)}
  {}

  void operator() (cl::sycl::item<1> itemId) {
    // local thread id, thread size, and data size
    auto thread_id     = itemId.get_id(0);
    auto total_threads = itemId.get_range()[0];
    auto array_size    = C.get_count();
...
int main() {
  ...
  // beginning of SYCL objects' scope
  {
    ...
    queue.submit([&](cl::sycl::handler &cgh) {
      ...
      cgh.parallel_for(cl::sycl::range<1>{total_threads}, vector_add(A_acc, B_acc, C_acc));
    });
